<?php
	include 'connect.php';
class dropborrowers_records extends connect
{
		...
		public function search($s)
	{
		...
		public function searching()
		{
			$field="$_POST[s1]";
			if($field=="all")
				$str="select * from borrowers_records";
			else
				$str="select * from borrowers_records where $_POST[s1]='$_POST[t1]' ";
		   $this->search($str);
		}

	
}
$obj=new dropborrowers_records();
if(isset($_REQUEST["b1"]))
	$obj->searching();

	echo "<link rel='stylesheet' href='borrowers_records.css'>";
	echo "<form name=f method=post action=dropborrowers_records.php>";
	echo "<table border=1>";
	echo "<tr>
